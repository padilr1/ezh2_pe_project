...
compare_means(PSI ~ condition, data = mouse_neuronal_differentiation,paired = FALSE,p.adjust.method = "fdr", method = 't.test') %>% dplyr::select(-".y.") %>% reactable()
```


```{r, echo = FALSE, fig.cap="Figure 1. Plotted as median +/- interquartile range"}
my_comparisons = list(c("mESC","NCC"),c("NCC","neuron"),c("mESC","neuron"))
...
ggplot(mouse_neuronal_differentiation,aes(condition,PSI)) +
