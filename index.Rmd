...
```{r, echo = FALSE, fig.cap="Figure 1. Plotted as median +/- interquartile range. mESC = mouse embryonic stem cells; NCC = neural crest cells"}
...
load("~/Documents/ezh2_pe_project/df_hsapien_neuron_diff.RData")

compare_means(PSI ~ condition, data = df_hsapien_neuron_diff,paired = FALSE,p.adjust.method = "fdr", method = 'wilcox.test',method.args = list(alternative = "greater"),ref.group = "iPSC") %>% dplyr::select(-".y.") %>% reactable()
```


```{r, echo=FALSE,warning=FALSE,fig.cap="Figure 2. Plotted as median ± interquartile range. iPSC = induced pluripotent stem cells; NPC = neural progenitor cell; Rosette = highly organized multicellular structures derived from NPC and neuroprogenitors within rosettes differentiate into neurons"}
#reorder boxplot
df_hsapien_neuron_diff$condition <- factor(df_hsapien_neuron_diff$condition, levels=c("iPSC","NPC","Rosette","Neuron"))

#subset_neuron <- df_hsapien_neuron_diff[df_hsapien_neuron_diff$condition == "neuron",]

summary_burke <- df_hsapien_neuron_diff %>%
  dplyr::group_by(condition) %>%
  summarise(
    se = IQR(PSI),
    PSI = median(PSI)
...
p_burke <- ggplot(df_hsapien_neuron_diff,aes(condition,PSI)) +
  geom_jitter(position = position_jitter(0.2),color = "darkgray", size = 2) +
...
stat.test_burke <- compare_means(PSI ~ condition, data = df_hsapien_neuron_diff,paired = FALSE,p.adjust.method = "fdr", method = 'wilcox.test',method.args = list(alternative = "greater"),ref.group = "iPSC") %>% dplyr::select(-".y.") %>%
