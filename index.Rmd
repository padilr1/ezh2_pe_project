...
compare_means(PSI ~ condition, data = mouse_neuronal_differentiation,paired = FALSE,p.adjust.method = "fdr", method = 'wilcox.test',method.args = list(alternative = "greater")) %>% dplyr::select(-".y.") %>% reactable()
```


```{r, echo = FALSE, fig.cap="Figure 1. Plotted as median +/- interquartile range"}
...
stat.test_mouse <- compare_means(PSI ~ condition, data = mouse_neuronal_differentiation,paired = FALSE,p.adjust.method = "fdr", method = 'wilcox.test',method.args = list(alternative = "greater")) %>% dplyr::select(-".y.") %>% mutate(y.position = c(18,22,20))

p_mouse + stat_pvalue_manual(stat.test_mouse, label = "p.adj = {p.adj}")
```
# Human iPSCs differentiated into neurons
```{r, echo=FALSE, warning=FALSE}
load("~/Documents/ezh2_pe_project/df_burke_2020.RData")

df_burke_2020$condition <- as.factor(df_burke_2020$condition)

df_burke_2020 <- df_burke_2020 %>%
  slice(-c(4))

compare_means(PSI ~ condition, data = df_burke_2020,paired = FALSE,p.adjust.method = "fdr", method = 'wilcox.test',method.args = list(alternative = "greater")) %>% dplyr::select(-".y.") %>% reactable()
```

```{r, echo=FALSE,warning=FALSE,fig.cap="Figure 2. Plotted as median ± interquartile range"}
summary_burke <- df_burke_2020 %>%
  dplyr::group_by(condition) %>%
  summarise(
    se = IQR(PSI),
    PSI = median(PSI)
...
p_burke <- ggplot(df_burke_2020,aes(condition,PSI)) +
  geom_jitter(position = position_jitter(0.2),color = "darkgray", size = 2) +
...
stat.test_burke <- compare_means(PSI ~ condition, data = df_burke_2020,paired = FALSE,p.adjust.method = "fdr", method = 'wilcox.test',method.args = list(alternative = "greater")) %>% dplyr::select(-".y.") %>%
  mutate(y.position = c(45,55,50))
